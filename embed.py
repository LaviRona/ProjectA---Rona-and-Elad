...
def embed_texts(
    texts: Sequence[str], *, batch_size: int = 256, progress: bool = False
) -> np.ndarray:
    """Return L2-normalized embeddings, shape (n, dim).

    Set ``progress=True`` (used by the offline build) to show a tqdm bar with
    elapsed time, ETA and items/sec. Left off at query time to keep run() quiet.
    """
    if not texts:
